...
class ViewPagerAct : SimpleActivity(), ViewPager.OnPageChangeListener, ViewPagerFrm.FragmentListener {
    ...
    fun refreshMenuItems() {
        ...
        runOnUiThread {
            ...
            binding.mediumViewerToolbar.menu.apply {
                ...
                findItem(R.id.menuEdit).isVisible = visibleBottomActions and BOTTOM_ACTION_EDIT == 0 && !currentMedium.isSVG()
                findItem(R.id.menuRename).isVisible = visibleBottomActions and BOTTOM_ACTION_RENAME == 0 && !currentMedium.getIsInRecycleBin()
                findItem(R.id.menuRotate).isVisible = currentMedium.isImage() && visibleBottomActions and BOTTOM_ACTION_ROTATE == 0
                findItem(R.id.menuSetAs).isVisible = visibleBottomActions and BOTTOM_ACTION_SET_AS == 0
                ...
                findItem(R.id.menuResize).isVisible = visibleBottomActions and BOTTOM_ACTION_RESIZE == 0 && currentMedium.isImage()
                ...
                findItem(R.id.menuRestoreFile).isVisible = currentMedium.path.startsWith(recycleBinPath)
                findItem(R.id.menuCreateShortcut).isVisible = isOreoPlus()
                findItem(R.id.menuChangeOrientation).isVisible = rotationDegrees == 0 && visibleBottomActions and BOTTOM_ACTION_CHANGE_ORIENTATION == 0
                findItem(R.id.menuChangeOrientation).icon = resources.getDrawable(getChangeOrientationIcon())
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mediumViewerToolbar.setOnMenuItemClickListener { menuItem ->
            ...
            when (menuItem.itemId) {
                ...
                R.id.menuRename -> checkMediaManagementAndRename()
                ...
                R.id.menuRotate180 -> rotateImage(180)
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putString(/* key = */ SAVED_PATH, /* value = */ getCurrentPath())
    }
    ...
    private fun initViewPager(savedPath: String) {
        ...
        if (mPath.isPortrait() && getPortraitPath() == "") {
            val newIntent = Intent(this, ViewPagerAct::class.java)
        ...
        binding.viewPager.onGlobalLayout {
            if (!isDestroyed) {
                if (mMediaFiles.isNotEmpty()) {
                    gotMedia(thumbnailItems = mMediaFiles as ArrayList<ThumbnailItem>, refetchViewPagerPosition = true)
    ...
    private fun updatePagerItems(media: MutableList<Medium>) {
        ...
        if (!isDestroyed) {
            ...
            binding.viewPager.apply {
                ...
                removeOnPageChangeListener(this@ViewPagerAct)
                adapter = pagerAdapter
                pagerAdapter.shouldInitFragment = true
                addOnPageChangeListener(this@ViewPagerAct)
                currentItem = mPos
            }
        }
    }
    ...
    private fun goToNextMedium(forward: Boolean) {
        ...
        if (newPosition == -1 || newPosition > binding.viewPager.adapter!!.count - 1) {
            ...
            binding.viewPager.setCurrentItem(/* position = */ newPosition, /* smoothScroll = */ false)
        }
    }
    ...
    private fun slideshowEnded(forward: Boolean) {
        if (config.loopSlideshow) {
            if (forward) {
                ...
                binding.viewPager.setCurrentItem(
                    /* position = */ binding.viewPager.adapter!!.count - 1,
                    /* smoothScroll = */ false
                )
            }
        } else {
    ...
    private fun createShortcut() {
        ...
        if (manager.isRequestPinShortcutSupported) {
            ...
            getShortcutImage(path, drawable) {
                val intent = Intent(this, ViewPagerAct::class.java).apply {
                    putExtra(PATH, path)
